 ...
 * @LastEditTime: 2021-06-05 00:48:39
 * @Description: 在gpu_fail.cu基础上使用共享内存
 * @FilePath: /src/gpu_shareMem.cu
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
/**
 * @brief 核函数，在gpu_fail.cu基础上使用共享内存
 * 
 * @param width 矩阵列数
 * @param height 矩阵行数
 * @param array 待计算矩阵
...
int main(int argc, char const *argv[])
{
    ...
    printf("矩阵维度%dx%d，使用共享内存在GPU上运行时间: %f ms.\n", height,width,time_gpu*1000);
    cudaFree(host_array);
